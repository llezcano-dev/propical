...
`;
  ...
  const migrations = [
    ...
    // per-property master toggle for cleaning logic. When 0,
...
`;
  ...
  // CleanerAssignment table — property ↔ cleaner profile.
  // Cleaners are metadata (the Cleaner pool); the legacy cleanerId /
  // role="cleaner" User path was removed.
  const cleanerSchema = `
...
`;
  ...
  // Cleaner profile table (account-level metadata,
...
`;
  ...
  // SeoOverride — per-page SEO overrides
...
`;
  ...
  // CalendarPlatform — platform preset registry
...
`;
  ...
  // GuestFormTemplate / GuestFormSubmission — pre-arrival guest forms
