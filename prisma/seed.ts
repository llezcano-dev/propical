...
import { assertSeedAllowsSuperadmin } from "../src/lib/seed-guard";
...
async function main() {
  ...
  // Refuse to create/reset a default-named "admin" superadmin in
  // production. Prevents a predictable superadmin account and, worse,
  // a password reset of an existing "admin" user with the new password
  // printed to stdout.
  assertSeedAllowsSuperadmin({ username });
