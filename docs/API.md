...
> ⚠️ **Desactualizado (2026-08-12).** Snapshot del upstream: le faltan ~25
> endpoints que existen hoy (auth email-verified, google OAuth, guest forms,
> message-templates, property-managers, cleaners, admin/*, feedback, etc.)
> y la lista de paths públicos se amplió. Migrar a generación automática
> (`scripts/gen-api-docs.ts`) — ver [TODO.md](TODO.md).

All endpoints are JSON unless noted. Base URL: `/`.

## Authentication

A successful `POST /api/auth/login` sets the `propical-session` cookie (HTTP-only, JWT).
...
On success: sets `propical-session` cookie; returns `{ user: { userId, username, role } }`.
