/**
 * Per-username account lockout. Layered on top of the per-IP
 ...
 * fine for single-instance deployments; on serverless each lambda has
 * fresh memory so lockouts don't persist across invocations). Lockouts
 * vanish on restart, which we accept — the auth flow still re-locks
