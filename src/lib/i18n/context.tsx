...
const LEGACY_LOCALSTORAGE_KEY = "propical-locale";
...
export function I18nProvider({
  ...
  useEffect(() => {
    ...
    // Browser language detection — map navigator.language to supported locale
    try {
      const browserLang = (navigator.language || "").split("-")[0];
      const LOCALE_MAP: Record<string, Locale> = { en: "en", pt: "pt", es: "es" };
      if (browserLang in LOCALE_MAP) {
        setLocaleState(LOCALE_MAP[browserLang]);
      }
    } catch {
      // SSR or restricted environment — keep default
    }
  }, [initialLocale]);
  ...
  const t = useCallback(
    (key: TranslationKey, params?: Record<string, string | number>) => {
      ...
      if (!entry) return key;
      let text: string = (entry as Record<Locale, string>)[locale] || entry.en;
      if (params) {
