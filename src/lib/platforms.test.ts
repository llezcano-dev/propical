...
describe("PLATFORM_PRESETS", () => {
  // Locked-in seed list per spec — guards against accidental drops.
  const expectedSlugs = [
    "airbnb",
    "booking",
    "vrbo",
...
describe("resolvePlatformMeta", () => {
  it('returns "Booking.com" displayName / "Booking" shortLabel for booking', () => {
    const m = resolvePlatformMeta("booking");
    expect(m.displayName).toBe("Booking.com");
    expect(m.shortLabel).toBe("Booking");
    expect(m.color).toBe("#003580");
    expect(m.slug).toBe("booking");
  });

  it("returns Direct for direct slug", () => {
    const m = resolvePlatformMeta("direct");
    expect(m.displayName).toBe("Direct");
    expect(m.shortLabel).toBe("Direct");
    expect(m.color).toBe(FALLBACK_PLATFORM_COLOR);
    expect(m.slug).toBe("direct");
  });

  it("returns Vrbo for vrbo slug", () => {
    const m = resolvePlatformMeta("vrbo");
    expect(m.displayName).toBe("Vrbo");
    expect(m.shortLabel).toBe("Vrbo");
    expect(m.color).toBe("#245ABC");
    expect(m.slug).toBe("vrbo");
  });

  it("returns Airbnb for airbnb slug", () => {
    const m = resolvePlatformMeta("airbnb");
    expect(m.displayName).toBe("Airbnb");
    expect(m.shortLabel).toBe("Airbnb");
    expect(m.color).toBe("#FF385C");
  });

  it("capitalises unknown slug and falls back to neutral gray", () => {
    const m = resolvePlatformMeta("custom-platform");
    expect(m.displayName).toBe("Custom-platform");
    expect(m.shortLabel).toBe("Custom-platform");
    expect(m.color).toBe(FALLBACK_PLATFORM_COLOR);
    expect(m.slug).toBe("custom-platform");
  });

  it("regression B2: direct resolves as Direct, not Airbnb", () => {
    const m = resolvePlatformMeta("direct");
    expect(m.displayName).toBe("Direct");
    expect(m.shortLabel).toBe("Direct");
    // Guard against the old ternary that defaulted everything to Airbnb.
    expect(m.displayName).not.toBe("Airbnb");
    expect(m.shortLabel).not.toBe("Airbnb");
  });
});
