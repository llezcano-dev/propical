...
export async function auditProperty(propertyId: number): Promise<PropertyAuditReport> {
  ...
  if (links.length === 0) {
    findings.push({
      ...
      category: "settings",
      message:
        "No calendar links connected. Bookings on Airbnb / Booking won't sync into Propical, and the iCal feed has nothing to merge from other platforms.",
    });
  }
  ...
  for (const link of links) {
    if (link.lastError) {
      findings.push({
        severity: "error",
        category: "feed",
        message: `Last sync from ${link.platform} failed: ${link.lastError}. Other-platform bookings on this URL aren't reaching Propical.`,
