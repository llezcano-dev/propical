...
describe("parseICal DATE vs DATE-TIME (B1)", () => {
  const calendar = (lines: string[]) =>
    ["BEGIN:VCALENDAR", ...lines, "END:VCALENDAR"].join("\r\n");

  it("VALUE=DATE and DATE-TIME with Z yield the same calendar date", () => {
    const asDate = parseICal(
      calendar(["BEGIN:VEVENT", "UID:d1", "SUMMARY:S", "DTSTART;VALUE=DATE:20260814", "DTEND;VALUE=DATE:20260816", "END:VEVENT"])
    );
    const asDateTime = parseICal(
      calendar(["BEGIN:VEVENT", "UID:d2", "SUMMARY:S", "DTSTART:20260814T000000Z", "DTEND:20260816T000000Z", "END:VEVENT"])
    );
    expect(asDate[0].startDate).toBe("2026-08-14");
    expect(asDate[0].endDate).toBe("2026-08-16");
    expect(asDateTime[0].startDate).toBe(asDate[0].startDate);
    expect(asDateTime[0].endDate).toBe(asDate[0].endDate);
  });

  it("floating DATE-TIME (no Z / no TZID) keeps the date component as-is", () => {
    const events = parseICal(
      calendar(["BEGIN:VEVENT", "UID:f1", "SUMMARY:S", "DTSTART:20260301T140000", "DTEND:20260305T110000", "END:VEVENT"])
    );
    expect(events[0].startDate).toBe("2026-03-01");
    expect(events[0].endDate).toBe("2026-03-05");
  });

  it("DATE-TIME with TZID keeps the local date component (no UTC conversion)", () => {
    const events = parseICal(
      calendar(["BEGIN:VEVENT", "UID:t1", "SUMMARY:S", "DTSTART;TZID=America/Sao_Paulo:20260814T020000", "DTEND;TZID=America/Sao_Paulo:20260816T110000", "END:VEVENT"])
    );
    // The event actually starts 2026-08-14 02:00 -03:00 = 05:00 UTC; the
    // parser must NOT shift it — the calendar date is the platform's
    // booking date, and Propical compares date-only strings everywhere.
    expect(events[0].startDate).toBe("2026-08-14");
    expect(events[0].endDate).toBe("2026-08-16");
  });

  it("single-day DATE-TIME without DTEND defaults endDate to startDate", () => {
    const events = parseICal(
      calendar(["BEGIN:VEVENT", "UID:s1", "SUMMARY:S", "DTSTART:20260601T000000Z", "END:VEVENT"])
    );
    expect(events[0].startDate).toBe("2026-06-01");
    expect(events[0].endDate).toBe("2026-06-01");
  });

  it("time component of DATE-TIME never changes the extracted day", () => {
    // Even a late-evening UTC DATE-TIME keeps its calendar date — the
    // date part, not a timezone-shifted instant, is the contract.
    const events = parseICal(
      calendar(["BEGIN:VEVENT", "UID:n1", "SUMMARY:S", "DTSTART:20260814T233000Z", "DTEND:20260816T010000Z", "END:VEVENT"])
    );
    expect(events[0].startDate).toBe("2026-08-14");
    expect(events[0].endDate).toBe("2026-08-16");
  });
...
describe("generateICal", () => {
  ...
  it("emits a far-past placeholder VEVENT when given an empty events list", () => {
    ...
    expect(parsed[0].uid).toBe("propical-placeholder");
