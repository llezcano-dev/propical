/**
 * Google OAuth 2.0 helpers + Google One Tap helpers.
 *
 ...
 *
 * Behind a reverse proxy (e.g. Vercel, nginx) `new URL(request.url)`
 * reflects the proxy hop's HTTP/localhost addressing, not the public-facing
 * URL the proxy presents to users. Honor X-Forwarded-Proto / X-Forwarded-Host
...
export function getPublicOrigin(request: Request): string {
  ...
  // (proxy chain can stack headers in some setups).
 ...
 *   https://propical.com.br/api/auth/google/callback
