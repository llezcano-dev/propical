...
export function AdminPanel() {
  ...
  return (
    <div className="space-y-8">
      <section>
        <h2 className={cn("mb-4", eyebrowVariants({ variant: "section" }))}>
          Admin · Site settings
        </h2>
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <h2 className={eyebrowVariants({ variant: "section" })}>
            Admin · Users
          </h2>
          <Button
            variant="ghost"
                    ...
                    return (
                      <TableRow key={u.id} className={isSuspended ? "opacity-60" : ""}>
                        <TableCell className="font-medium">
                          {u.username}
                          {isSuspended && (
                            <span className="ml-2 inline-flex items-center rounded-md bg-destructive/15 px-1.5 py-0.5 text-sm font-medium uppercase tracking-wide text-destructive">
                              suspended
                            </span>
                          )}
                        </TableCell>
      ...
      <section>
                    ...
                    return (
                      <TableRow key={p.slug} className={!draft.enabled ? "opacity-60" : ""}>
                        <TableCell className="font-mono text-xs text-muted-foreground">
                          {p.slug}
                          {p.isCustom && (
                            <span className="ml-2 inline-flex items-center rounded-md bg-primary/15 px-1.5 py-0.5 text-sm font-medium uppercase tracking-wide text-primary">
                              custom
                            </span>
                          )}
                        </TableCell>
        ...
        <div className="mt-4 rounded-xl border border-border/60 bg-card/50 p-5">
          ...
          <p className="mb-4 text-xs text-muted-foreground/70">
            ...
            baked into the outbound iCal feed URL <code className="font-mono text-sm">/for-{`{slug}`}.ics</code>.
          </p>
      ...
      <section>
                  ...
                  >
                    <summary className="flex cursor-pointer items-center justify-between gap-3 text-sm">
                      <span className="font-mono text-xs">
                        {row.path}
                        <span className="ml-2 inline-flex items-center rounded-md bg-muted/40 px-1.5 py-0.5 text-sm uppercase tracking-wide text-muted-foreground">
                          {row.locale}
                        </span>
                      </span>
        ...
        <div className="mt-4 rounded-xl border border-border/60 bg-card/50 p-5">
          ...
          <p className="mb-4 text-xs text-muted-foreground/70">
            Path is the URL pathname (e.g. <code className="font-mono text-sm">/about</code>).
