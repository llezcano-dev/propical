...
const HINT_COPY: Record<Locale, HintCopy> = {
  ...
  pt: {
    title: "Dica — formulário pré-chegada.",
    before:
      "Você pode configurar um formulário para o hóspede (dados do passaporte, hora de chegada, perguntas) em",
    link: "a página Formulário pré-chegada",
    after:
      ". Uma vez configurado, um link para compartilhar aparecerá aqui em cada reserva.",
  },
  es: {
 ...
 *  checkIn/checkOut are UTC instants; UTC reads keep the day/month/year
 *  in the calendar-date frame no matter the host's timezone (B1 class —
 *  local getters shift the day in non-UTC timezones). */
function formatStayRange(checkInIso: string, checkOutIso: string): string {
  ...
  const ciMon = ci.toLocaleDateString("en-GB", { timeZone: "UTC", month: "long" });
  const coMon = co.toLocaleDateString("en-GB", { timeZone: "UTC", month: "long" });
...
export function ReservationView({
  ...
  // pre-arrival guest form share link. hasGuestForm gates
  ...
  // + clipboard copy — Propical doesn't send anything to the guest;
  ...
  // place it on the clipboard. Propical does NOT send anything to the
  ...
  const guestFormStatusLabel = (): string | null => {
    ...
    if (guestFormSubmission.submittedAt) {
      const days = Math.max(
        0,
        Math.round(
          (now - new Date(guestFormSubmission.submittedAt).getTime()) /
            (1000 * 60 * 60 * 24)
        )
      );
      return days === 0
        ? tr("reservation.formSubmittedToday")
        : tr(days === 1 ? "reservation.formSubmittedDay" : "reservation.formSubmittedDays", { days });
    }
    // Avoid the word "Sent" — Propical doesn't send the link, the host
    // does. We only know when the share token was minted (i.e. when the
    // link was first copied to the clipboard).
    const days = Math.max(
      0,
      Math.round(
        (now - new Date(guestFormSubmission.sentAt).getTime()) /
          (1000 * 60 * 60 * 24)
      )
    );
    return days === 0
      ? tr("reservation.linkGeneratedToday")
      : tr(days === 1 ? "reservation.linkGeneratedDay" : "reservation.linkGeneratedDays", { days });
  };
  ...
  const formatIsoDate = (s: string) => toUtcDateStr(new Date(s));
  ...
  const formatDate = (dateStr: string) => {
    return new Date(dateStr).toLocaleDateString("en-GB", {
      ...
      month: "short",
      year: "numeric",
      // UTC read — dateStr is a UTC instant; local rendering shifts the
      // displayed day in non-UTC timezones (B1 class).
      timeZone: "UTC",
    });
  };
  ...
  const exportGuestsCsv = () => {
    ...
    a.href = url;
    const dateStr = toUtcDateStr(new Date(reservation.checkIn));
  ...
  return (
    ...
    <div className="mx-auto max-w-[1760px] space-y-6 px-3 sm:px-5">
          ...
          {editing ? (
            <div className="space-y-3 rounded-xl border border-border/60 bg-card/50 p-4">
              ...
              <div className="flex gap-2">
                <Button size="sm" className="rounded-lg text-sm" onClick={handleSaveEdit}>
                  Save
                </Button>
                <Button variant="ghost" size="sm" className="rounded-lg text-sm" onClick={() => setEditing(false)}>
                  Cancel
                </Button>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <button
                  onClick={() => {
                    if (window.confirm(tr("reservation.deleteConfirm", { name: reservation.name }))) {
                      onDeleteReservation(reservation.id);
                    }
                  }}
                  aria-label={tr("reservation.deleteReservation")}
                  title={tr("reservation.deleteReservation")}
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-2">
                <span
                  data-testid="res-platform-badge"
                  className="rounded-md px-2 py-0.5 text-sm font-semibold text-white"
                  style={{ backgroundColor: resolvePlatformMeta(reservation.platform).color }}
                >
                  {resolvePlatformMeta(reservation.platform).displayName}
                </span>
                <span className="text-caption">
                  {formatDate(reservation.checkIn)} — {formatDate(reservation.checkOut)}
                </span>
                <Badge variant="outline" className="rounded-md text-sm">
                  {stayNights()} {stayNights() === 1 ? tr("common.night") : tr("common.nights")} · {stayDays()} {stayDays() === 1 ? tr("common.day") : tr("common.days")}
                </Badge>
                {guests.length > 0 && (
      ...
      {/* Pre-arrival form share link. Hidden when the property
          has no GuestFormTemplate configured (set up under Sync Settings).
          Action is generate + copy: Propical doesn't send anything to
        ...
        <div className="rounded-xl border border-dashed border-border/50 px-4 py-2.5">
          <p className="text-caption">
            ...
            <Link
              ...
              className="font-medium text-action-primary-text underline underline-offset-2 hover:text-action-primary-text-hover"
            >
              {hint.link}
            </Link>
            {hint.after}
      ...
      <div className="space-y-2 rounded-xl border border-border/60 bg-card/40 px-4 py-3">
        ...
        <div className="space-y-1 border-t border-border/30 pt-2">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-sm transition-opacity ${reservationPhoneState === "idle" ? "opacity-0" : "opacity-100"} ${
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <input
              ...
              className="min-w-0 flex-1 rounded-md border border-border/40 bg-background/50 px-2 py-1 text-sm text-text-primary placeholder-muted-foreground/30 outline-none focus:border-primary/60"
            />
            <a
              href={
              ...
              className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md transition-all ${
                reservationPhoneEnabled
                  ? "bg-messenger-whatsapp/15 text-messenger-whatsapp hover:bg-messenger-whatsapp/25"
            ...
            <a
              ...
              className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md transition-all ${
                reservationPhoneEnabled
                  ? "bg-messenger-telegram/15 text-messenger-telegram hover:bg-messenger-telegram/25"
                  : "bg-white/5 text-muted-foreground/30 cursor-not-allowed"
              }`}
            >
              <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="currentColor" aria-hidden="true">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-md border border-border/50 bg-background/50 px-2 py-1 text-sm text-muted-foreground outline-none transition-colors focus:border-primary/50 focus:text-foreground"
          />
        ...
        {!editingGroupUrls ? (
          <div className="flex flex-wrap items-center gap-2 pt-1">
            {reservation.tgGroupUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-md border border-messenger-telegram/30 bg-messenger-telegram/10 px-3 py-1.5 text-sm font-medium text-messenger-telegram transition-colors hover:bg-messenger-telegram/20"
              >
            ...
            {reservation.waGroupUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-md border border-messenger-whatsapp/30 bg-messenger-whatsapp/10 px-3 py-1.5 text-sm font-medium text-messenger-whatsapp transition-colors hover:bg-messenger-whatsapp/20"
              >
                <svg viewBox="0 0 24 24" className="h-3 w-3" fill="currentColor" aria-hidden="true">
                  <path d="M20.52 3.48A11.78 11.78 0 0012.05 0C5.5 0 .2 5.3.2 11.85c0 2.09.55 4.13 1.6 5.93L0 24l6.39-1.67a11.85 11.85 0 005.66 1.44h.01c6.55 0 11.85-5.3 11.85-11.85 0-3.16-1.23-6.13-3.39-8.44zM12.06 21.7h-.01a9.84 9.84 0 01-5.02-1.37l-.36-.21-3.79.99 1.01-3.69-.23-.38a9.83 9.83 0 01-1.5-5.19c0-5.44 4.42-9.86 9.87-9.86 2.63 0 5.11 1.03 6.97 2.89a9.79 9.79 0 012.89 6.97c-.01 5.45-4.43 9.85-9.83 9.85zm5.4-7.38c-.3-.15-1.75-.86-2.02-.96-.27-.1-.47-.15-.66.15-.2.3-.76.96-.93 1.16-.17.2-.34.22-.64.07-.3-.15-1.25-.46-2.38-1.46a8.92 8.92 0 01-1.65-2.05c-.17-.3-.02-.46.13-.61.13-.13.3-.34.45-.51.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.07-.15-.66-1.6-.91-2.18-.24-.58-.49-.5-.66-.51-.17-.01-.37-.01-.57-.01-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48 0 1.46 1.06 2.87 1.21 3.07.15.2 2.09 3.19 5.07 4.47.71.3 1.26.49 1.69.62.71.23 1.36.2 1.87.12.57-.08 1.75-.71 2-1.4.25-.69.25-1.27.17-1.4-.07-.13-.27-.2-.57-.35z" />
                </svg>
          ...
          <div className="space-y-2 pt-1">
            <label className="flex items-center gap-2">
              <span className="w-20 shrink-0 text-sm font-medium text-muted-foreground">Telegram</span>
              <input
                type="url"
                value={editTgGroupUrl}
                onChange={(e) => setEditTgGroupUrl(e.target.value)}
                placeholder="https://t.me/+abcdef…"
                className="h-9 flex-1 rounded-md border border-border/50 bg-background/50 px-2 text-sm outline-none focus:border-primary/50"
              />
            </label>
            <label className="flex items-center gap-2">
              <span className="w-20 shrink-0 text-sm font-medium text-muted-foreground">WhatsApp</span>
              <input
                type="url"
            ...
            {groupUrlsError && (
              <p className="text-sm text-rose-500">{groupUrlsError}</p>
            )}
      ...
      <div className="flex items-start gap-2 rounded-lg border border-border/40 bg-card/20 px-3 py-2">
        ...
        <p className="text-sm leading-relaxed text-muted-foreground">
          Passport documents are private to your account. Propical support and
          administrators cannot view your guests&apos; passport details.
        </p>
      </div>
          ...
          {templates.length > 0 && (
            <div className="relative" ref={templateMenuRef}>
              <button
                ...
                className="flex items-center gap-1.5 rounded-md border border-border/30 bg-card/30 px-3 py-1.5 text-sm font-medium text-text-muted transition-all hover:border-border/60 hover:bg-card/60 hover:text-text-primary"
              ...
              {templatePickerOpen && (
                <div className="absolute right-0 z-20 mt-1 w-64 overflow-hidden rounded-md border border-border bg-surface shadow-lg">
                  <ul className="max-h-72 overflow-y-auto py-1">
                    {templates.map((t) => (
                      <li key={t.id}>
                        <button
                          onClick={() => pickTemplate(t)}
                          className="flex w-full items-start gap-2 px-3 py-2 text-left text-sm text-text-primary hover:bg-surface-raised"
                        >
                          <span className="mt-0.5 rounded bg-border-strong px-1.5 py-0.5 text-sm uppercase text-text-muted">
                            {t.language}
                          </span>
        ...
        <div className="rounded-xl border border-border bg-surface">
          <div className="flex items-center justify-between border-b border-border/30 px-4 py-2.5">
            <div className="flex items-center gap-2 text-sm">
              <span className="font-medium text-text-primary">{activeTemplate.name}</span>
              <span className="rounded bg-border-strong px-1.5 py-0.5 text-sm uppercase text-text-muted">
          ...
          <div className="space-y-2 p-4 text-sm">
            {renderedSubject && (
              <div className="font-semibold text-text-primary">{renderedSubject}</div>
            )}
            <textarea
              ...
              className="w-full resize-y rounded-md border border-border bg-surface px-3 py-2 font-sans text-text-secondary outline-none focus:border-border-strong"
            />
          </div>
        </div>
      )}
      ...
      {/* Pre-arrival answers panel. Renders the submitted
