...
const COPY: Record<Locale, CopyShape> = {
  ...
  pt: {
    dateLocale: "pt-BR",
    arriveByLabel: (time) => `limpeza rápida, chegar antes das ${time}`,
    fullDayLabel: "dia inteiro disponível",
    manualLabel: "limpeza",
    potentialPrefix: "possível — ",
    scheduleHeader: "Agenda de limpezas",
    conflictNoBackup: " (conflito — sem substituto configurado)",
    conflictBackupBusy: (name) => ` (conflito — o substituto ${name} também ocupado)`,
    conflictBackupSet: (name) => ` (conflito — substituto: ${name})`,
    daysCount: (n) => (n === 1 ? "dia" : "dias"),
    cleaningCta: "Limpeza",
  },
  es: {
    dateLocale: "es-ES",
...
/** single cleaner assignment slot for a property.
...
interface CleaningScheduleHandle {
  copy: () => void;
  print: () => void;
}
...
interface CleaningDay {
  ...
  // name of the priority-0 cleaner assigned to the
  ...
  cleanerName?: string;
  // stable identity key of the priority-0 cleaner.
...
interface CleaningScheduleProps {
  ...
  /** map of propertyId → ordered cleaner assignments
  ...
  /** fired with the sorted set of dates that contain a
...
/** Local-calendar-date of the current wall-clock day. Local read is
 *  deliberate here: "today" is the host's on-screen day, and the cutoff
 *  only compares against calendar-date strings (iCal / reservation
 *  dates), so it must stay in the local frame. */
function toLocalTodayStr(d: Date): string {
...
function computeCleaningDays(
  ...
  for (const res of property.reservations) {
    // Reservations carry UTC instants (2026-08-14T00:00:00.000Z).
    // Read via UTC getters so the calendar date survives any timezone —
    // local getters here shift the stay a day (B1 class bug).
...
export const CleaningSchedule = forwardRef<CleaningScheduleHandle, CleaningScheduleProps>(function CleaningScheduleImpl({
  ...
  const cleaningDays = useMemo(() => {
    ...
    for (const prop of targetProperties) {
      // properties with the cleaning toggle off contribute no rows.
  ...
  // cleaner conflicts. Group cleaning rows by
  ...
  const todayStr = toLocalTodayStr(new Date());
  ...
  const buildScheduleLines = (cleanerKey?: string): string[] => {
    ...
    for (const day of visibleDays) {
      ...
      // cleaner-conflict suffix. Mirrors the in-app
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && futureOverlaps.length > 0 && (
        <div className="rounded-lg border border-status-buffer-border bg-status-buffer-bg p-4 space-y-2">
          <div className="flex items-center gap-2">
            <svg className="h-5 w-5 text-status-buffer-fg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
            </svg>
            <span className="text-sm font-semibold text-status-buffer-fg">
              {t("cleaning.overlapWarning")} ({futureOverlaps.length} {c.daysCount(futureOverlaps.length)})
            </span>
          </div>
          <p className="text-caption text-status-buffer-fg opacity-80">
            {t("cleaning.overlapDesc")}
          </p>
          {futureOverlaps.map(o => (
            <div key={o.date} className="flex items-center gap-3 text-sm">
              <span className="font-medium text-text-primary">{formatDate(o.date)}</span>
              <span className="text-text-muted">{o.properties.join(" + ")}</span>
      ...
      {/* Cleaner conflicts — same cleaner is the
      ...
      {!loading && futureCleanerConflicts.length > 0 && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 space-y-2">
          ...
          {futureCleanerConflicts.map((c) => (
            <div key={`${c.date}-${c.cleanerKey}`} className="text-sm space-y-0.5">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                <span className="font-medium text-text-primary">{formatDate(c.date)}</span>
              ...
              {c.properties.map((p) => (
                <div key={`hint-${c.date}-${c.cleanerKey}-${p.id}`} className="pl-4 text-sm text-text-muted">
                  {(() => {
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {mode === "property" && selectedPropertyId && (
                <Link
                  ...
                  className="flex items-center gap-1.5 rounded-md bg-action-primary px-2.5 py-1.5 text-sm font-medium text-action-primary-fg transition-colors hover:bg-action-primary-hover"
                >
              ...
              {visibleDays.length > 0 && (
                <>
                  <button
                    ...
                    aria-label={t("cleaning.copySchedule")}
                    className="flex h-7 w-7 items-center justify-center rounded-md border border-border-strong bg-border-strong text-text-secondary transition-colors hover:bg-border-strong"
                  >
                    {copiedKey === "all" ? (
                  ...
                  <button
                    ...
                    title={t("cleaning.printSchedule")}
                    aria-label={t("cleaning.printSchedule")}
                    className="flex h-7 w-7 items-center justify-center rounded-md border border-border-strong bg-border-strong text-text-secondary transition-colors hover:bg-border-strong"
                  >
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {scheduleCleaners.map(([key, name]) => (
                    <button
                      ...
                      title={t("cleaning.copySchedule")}
                      className="flex items-center gap-1.5 rounded-md border border-border-strong bg-border-strong px-2.5 py-1.5 text-sm text-text-secondary transition-colors hover:bg-border-strong"
                    >
                      <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="divide-y divide-border/50">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-3">
                <div className="h-3 w-24 rounded bg-border-strong/60 animate-pulse" />
                <div className="h-3 w-16 rounded bg-border-strong/40 animate-pulse" />
                <div className="h-3 flex-1 max-w-[280px] rounded bg-border-strong/30 animate-pulse" />
              </div>
            ))}
          </div>
        ) : visibleDays.length === 0 ? (
              ...
              {visibleDays.map((day, i) => {
                ...
                return (
                  <div key={`m-${day.date}-${day.propertyId}-${i}`}>
                    {showYearDivider && (
                      <div className="border-b border-border bg-surface-hover px-4 py-2 text-sm font-semibold text-text-muted">
                        {thisYear}
                      </div>
                    )}
                    <div className={`flex flex-col gap-1.5 border-b border-border/50 px-4 py-2 ${isOverlap ? "bg-status-buffer-cell" : isCleanerConflict ? "bg-amber-500/10" : ""}`}>
                      {/* Line 1: date + badges, cleaner on the right */}
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-sm font-medium text-text-primary whitespace-nowrap">
                          {formatDate(day.date)}
                          {isOverlap && (
                            <span className="ml-1.5 text-sm font-medium text-status-buffer-fg">
                              {t("cleaning.overlap")}
                            </span>
                          )}
                          {!isOverlap && isCleanerConflict && (
                            <span className="ml-1.5 text-sm font-medium text-amber-300">
                              ⚠ {t("cleaning.cleanerConflictShort")}
                            </span>
                          )}
                        </span>
                        {day.cleanerName && (
                          <span className="inline-flex shrink-0 items-center gap-1 rounded bg-surface-hover px-1.5 py-0.5 text-sm font-medium text-text-secondary">
                            🧹 {day.cleanerName}
                          </span>
                        )}
                      </div>
                      {/* Line 2: all chips in one wrap */}
                      <div className="flex flex-wrap items-center gap-1.5 text-sm">
                        <span className={`inline-block rounded px-1.5 py-0.5 font-medium ${
                          day.type === "cleaning"
                            ? "bg-status-buffer-bg text-status-buffer-fg border border-status-buffer-border"
                            : "bg-text-primary/10 text-text-primary"
                        }`}>
                          {day.type === "cleaning" ? t("cleaning.typeClean") : t("cleaning.typePotential")}
                        </span>
                        {mode === "dashboard" && (
                          <span className="text-text-muted">{day.property}</span>
                        )}
                        {day.isManual && (
                          <span className="inline-block rounded bg-text-primary/10 px-1.5 py-0.5 font-medium text-text-primary">
                            {t("cleaning.manual")}
                          </span>
                        )}
                        {!day.isManual && (
                        ...
                        <span className="text-text-secondary">{formatReason(day)}</span>
                      </div>
                    </div>
                  </div>
                );
            ...
            <table className="hidden w-full sm:table">
              <thead className={cn("border-b border-border", eyebrowVariants({ variant: "tag" }))}>
                <tr className="text-left">
                  <th className="px-3 py-2 font-medium w-[140px]">{t("cleaning.date")}</th>
                  <th className="px-3 py-2 font-medium">{t("cleaning.type")}</th>
                  {mode === "dashboard" && (
                    <th className="px-3 py-2 font-medium">{t("cleaning.property")}</th>
                  )}
                  <th className="px-3 py-2 font-medium">{t("cleaning.notes")}</th>
                </tr>
              </thead>
              <tbody>
                {visibleDays.map((day, i) => {
                  ...
                  return (
                    <>{showYearDivider && (
                      <tr key={`year-${thisYear}`} className="border-b border-border">
                        <td colSpan={10} className="px-4 py-2 text-sm font-semibold text-text-muted bg-surface-hover">{thisYear}</td>
                      </tr>
                    )}
                    <tr key={`${day.date}-${day.propertyId}-${i}`} className={`border-b border-border/50 ${isOverlap ? "bg-status-buffer-cell" : isCleanerConflict ? "bg-amber-500/10" : "hover:bg-surface-hover"}`}>
                      <td className="px-3 py-1.5 text-sm text-text-primary whitespace-nowrap">
                        {formatDate(day.date)}
                        {isOverlap && <span className="ml-1.5 text-sm text-status-buffer-fg font-medium">{t("cleaning.overlap")}</span>}
                        {!isOverlap && isCleanerConflict && (
                          <span className="ml-1.5 text-sm text-amber-300 font-medium">⚠ {t("cleaning.cleanerConflictShort")}</span>
                        )}
                      </td>
                      <td className="px-3 py-1.5">
                        <span className={`inline-block rounded px-1.5 py-0.5 text-sm font-medium ${
                          day.type === "cleaning"
                            ? "bg-status-buffer-bg text-status-buffer-fg border border-status-buffer-border"
                            : "bg-text-primary/10 text-text-primary"
                        }`}>
                      ...
                      {mode === "dashboard" && (
                        <td className="px-3 py-1.5 text-sm text-text-muted">{day.property}</td>
                      )}
                      <td className="px-3 py-1.5 text-sm text-text-muted">
                        <div className="flex flex-wrap items-center gap-1.5">
                          {day.isManual && (
                            <span className="inline-block rounded bg-text-primary/10 px-1.5 py-0.5 text-text-primary font-medium">
                              {t("cleaning.manual")}
                            </span>
                          )}
                          {!day.isManual && (
                          ...
                          <span className="text-text-secondary">{formatReason(day)}</span>
                          {day.cleanerName && (
                            <span className="ml-auto inline-flex items-center gap-1 rounded bg-surface-hover px-1.5 py-0.5 text-sm font-medium text-text-secondary">
                              🧹 {day.cleanerName}
                            </span>
                          )}
                        </div>
