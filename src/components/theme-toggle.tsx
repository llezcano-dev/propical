...
export function ThemeToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`flex h-8 w-8 items-center justify-center rounded-md text-text-muted hover:text-text-primary hover:bg-surface-raised transition-colors ${className}`}
    >
      {theme === "light" ? (
