...
export function SyncAlertsBanner() {
  ...
  // Alerts carry a legacy "[ALERT]" marker outside the structured
  // payload — renderSyncLogMessage parses it and translates the message.
  const renderAlert = (message: string) =>
    renderSyncLogMessage(message, (key, params) =>
      t(key as TranslationKey, params),
    );

  const summary =
    alerts.length === 1
      ? renderAlert(alerts[0].message)
    ...
    >
      <div className="mx-auto flex max-w-6xl items-start justify-between gap-3">
        <div className="flex-1">
          ...
          {alerts.length > 1 && (
            <ul className="mt-1 list-disc space-y-0.5 pl-5 text-sm text-rose-700 dark:text-rose-200">
              {alerts.slice(0, 5).map((a) => (
                <li key={a.id}>{renderAlert(a.message)}</li>
              ))}
            </ul>
          )}
        </div>
        <button
          ...
          onClick={dismiss}
          disabled={dismissing}
          className="shrink-0 rounded border border-rose-300/60 px-2 py-1 text-sm text-rose-800 transition hover:bg-rose-100 disabled:opacity-50 dark:border-rose-900/60 dark:text-rose-300 dark:hover:bg-rose-900/40"
        >
