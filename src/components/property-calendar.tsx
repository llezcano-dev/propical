...
import { addDaysStr, capitalizeFirst } from "@/components/calendar/utils";
...
const COPY: Record<Locale, CopyShape> = {
  ...
  pt: {
    ...
    connectCalendar: "Conectar um calendário",
    dateLocale: "pt-BR",
    syncNow: "Sincronizar agora",
    syncing: "Sincronizando…",
    syncDone: "Calendário atualizado",
    syncCooldown: (s) => `Sincronização disponível em ${s} s`,
    pickADay: "Escolha um dia",
    pickADayHint: "Clique em qualquer data do calendário para abrir suas ações ou criar uma reserva.",
    today: "Hoje",
  },
  es: {
...
interface PropertyCalendarProps {
   ...
   *  is out of view. */
  properties: Property[];
  /** Re-fetch the page-level `properties` state so reservation rows
   *  (extends / trims done via PATCH inside this component) reach the
   *  calendar's bar computation immediately. Calendar mutations that
   *  go through `onAddReservation` already trigger it page-side; the
   *  direct PATCH paths below don't, which is why the bars stayed
   *  stale until F5 / the 60 s live-refresh poll. */
  onRefreshProperties?: () => void | Promise<void>;
...
export function PropertyCalendar({
  ...
  const { syncedEvents, links, overrides, loadingEvents, syncing, lastSyncAt, syncJustDone, refetchCalendarData, refetchOverrides, handleSyncNow } =
    useCalendarFetch(property.id, onRefreshProperties);

  // Live cooldown countdown for the "Sync now" button. `now` uses a
  // 1-second interval while cooldown is active so the remaining-seconds
  // label re-renders without a permanent interval. Timestamp is computed
  // inside state (not Date.now() in render) to satisfy React 19 purity.
  const [now, setNow] = useState(() => Date.now());
  const syncCooldownRemaining = lastSyncAt
    ? Math.max(0, Math.ceil((lastSyncAt + SYNC_COOLDOWN_MS - now) / 1000))
    : 0;
  useEffect(() => {
  ...
  void now;
  // now is read so the effect's dependency on syncCooldownRemaining
  ...
  // fetch the priority-0 cleaner so the cleaning chips
  ...
  useEffect(() => {
    ...
    fetch(`/api/cleaner-assignments?propertyId=${property.id}`)
      ...
      .then((list) => {
        ...
        const top = arr[0];
        setDefaultCleanerName(top?.cleanerName ?? undefined);
      })
      .catch(() => {
  ...
  const createReservationFromSelection = async (
    data: { name: string; platform: string },
  ): Promise<
    | { ok: true }
    | {
        ok: false;
  ...
  > => {
    ...
    if (sortedDates.length === 0) return { ok: false, error: "No dates selected" };
    ...
    if (!result.ok) {
      return { ok: false, error: result.error, existing: result.existing };
    }
    clearSelection();
  ...
  const extendBooking = async (rangeStart: string, rangeEnd: string, booking: ExtendableBooking) => {
    ...
    if (booking.reservationId) {
      ...
      clearSelection();
      await refetchCalendarData();
      // The PATCH moved the reservation's checkIn/checkOut, but that
      // row lives in the page-level `properties` state — refetch it so
      // the calendar's bars recompute with the new range right away
      // instead of waiting for F5 / the 60 s live-refresh poll.
    ...
    if (!result.ok) {
      window.alert(result.error || "Couldn't extend booking");
      return;
    }
    clearSelection();
    await refetchCalendarData();
  ...
  const trimReservation = async (reservationId: number, newCheckOut: string) => {
    ...
    clearSelection();
    await refetchCalendarData();
    // Same as extendBooking: the PATCH changed a reservation row that
    // only lives in the page-level `properties` state. Refetch so the
    // bar shrinks immediately.
    await onRefreshProperties?.();
  };

  const claimSyncedBooking = async (
    name: string,
  ): Promise<{ ok: boolean; error?: string }> => {
    if (!claimBar) return { ok: false, error: "No claim bar" };
    const result = await onAddReservation({
      name,
    ...
    closeClaim();
    await refetchCalendarData();
    // The page-level fetchProperties already ran inside handleAddReservation,
    // so handleSelectReservation (page L266-269) finds the reservation by its
    // fresh id from the POST response.
    onSelectReservation(result.reservation.id);
    return { ok: true };
  };
    ...
    <div className="mx-auto max-w-[1760px] px-3 sm:px-5 flex flex-col lg:flex-row gap-6">
      <div className={`min-w-0 space-y-6 lg:flex-1 ${panelOpen ? "hidden lg:block" : ""}`}>
        ...
        <div className="block">
          ...
          <header
            ref={stickyHeaderRef}
            className="sticky top-0 z-30 -mx-3 sm:-mx-4 px-3 sm:px-4 bg-surface mb-2 sm:mb-3 shadow-[0_8px_18px_-10px_rgba(0,0,0,0.18),0_2px_4px_-2px_rgba(0,0,0,0.04)]"
          >
            <div className="flex items-center justify-between gap-3 pt-2 sm:pt-3 pb-1">
              <h2 className="text-lg sm:text-2xl font-bold tracking-tight text-text-primary truncate">
                {capitalizeFirst(activeMonth.toLocaleDateString(c.dateLocale, {
                  month: "long",
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {activeMonthIdx !== PAST_MONTHS && (
                  <button
                    ...
                    title={c.today}
                    aria-label={c.today}
                    className="rounded-md border border-border-strong bg-surface-raised px-2.5 py-1 text-sm font-medium text-text-secondary transition-colors hover:bg-surface-hover hover:text-text-primary"
                  >
                    {c.today}
                  </button>
                )}
                ...
                {(syncing || syncJustDone) && (
                  <span
                    className={`hidden sm:inline-flex items-center gap-1 rounded-md px-2 py-1 text-sm font-medium ${
                      syncJustDone
                ...
                <button
                  ...
                  className="rounded-full p-1.5 sm:p-2 text-text-muted transition-colors hover:bg-surface-hover hover:text-text-primary disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent"
                >
            ...
            <div className="grid grid-cols-7 pb-2">
              {WEEKDAYS.map((wd) => (
                <div
                  key={wd}
                  className={cn("py-1 text-center", eyebrowVariants({ variant: "semibold" }))}
                >
          ...
          {months.map((m, i) => {
            ...
            const showYear = true;
            const monthLabel = capitalizeFirst(m.toLocaleDateString(c.dateLocale, {
              month: "long",
            ...
            >
              {/* In-flow month label so each month is visible at its
                  ...
                  their in-flow label. */}
              {i !== PAST_MONTHS && (
                <h3 className="mb-2 sm:mb-3 text-base sm:text-xl font-semibold tracking-tight text-text-secondary">
                  {monthLabel}
                </h3>
              )}
              <div className="rounded-lg border border-border bg-surface-raised [overflow:clip] [overflow-clip-margin:12px]">
                <CalendarGrid
                  year={m.getFullYear()}
                  month={m.getMonth()}
                  today={today}
      ...
      <aside
        className={`w-full lg:w-[400px] lg:shrink-0 lg:sticky lg:top-3 lg:self-start lg:max-h-[calc(100vh-84px)] rounded-2xl bg-surface shadow-[0_1px_3px_-1px_rgba(0,0,0,0.04),0_4px_16px_-8px_rgba(0,0,0,0.06)] [overflow:clip] ${
      ...
      >
        {panelOpen ? (
          ...
          <div className="flex h-full min-h-[320px] flex-col gap-5 px-6 py-8">
            {/* Property switcher removido — el selector del navbar
                (top-bar) ya cubre el cambio de propiedad; duplicarlo
                llenaba la vista de badges con muchas propiedades. */}
            <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-action-primary/10 text-action-primary-text">
