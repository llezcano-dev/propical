...
import { Eyebrow, eyebrowVariants } from "@/components/ui/atoms/eyebrow";
...
const COPY: Record<Locale, CopyShape> = {
  ...
  pt: { saving: "Salvando…", saved: "Salvo" },
...
interface GuestCardsProps {
  ...
  checkIn: string;
  checkOut: string;
  // used to build the WhatsApp prefill ("…this is {property},
  ...
  return (
    <div
      ...
      className={`group/field flex cursor-pointer items-center justify-between rounded-md px-2 py-1 transition-all ${
        highlighted
          ? "bg-text-primary/10 ring-1 ring-text-primary/20"
          : "hover:bg-white/5"
      }`}
    >
      <span className={`text-sm ${highlighted ? "text-text-primary/80" : "text-muted-foreground/60"}`}>{label}</span>
      <div className="flex items-center gap-1.5">
        <span className={`text-sm font-medium ${highlighted ? "text-text-primary" : ""}`}>{value || "—"}</span>
        <span className={`text-sm transition-all ${
          justCopied ? "text-emerald-500" : highlighted ? "text-text-primary/40" : "text-muted-foreground/0 group-hover/field:text-muted-foreground/30"
        }`}>
  ...
  return (
    <div className="flex items-center justify-between rounded-md px-2 py-1">
      ...
      <input
        ...
        className="w-2/3 rounded border border-border/40 bg-background/50 px-2 py-0.5 text-sm font-medium text-text-primary focus:border-primary/60 focus:outline-none"
      />
    </div>
  );
}
...
function GuestCard({
  ...
  // per-guest notes auto-save. Local draft mirrors the
  ...
  // phone auto-save. Same draft/saved/state pattern as notes
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border/20 px-3 py-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className="truncate text-sm font-semibold text-text-primary">{guest.fullName}</span>
          <span className="shrink-0 text-sm text-text-muted">{effectiveAge(guest)}y</span>
        </div>
          ...
          {editing ? (
            <>
              <button
                onClick={saveEdit}
                disabled={saving}
                className="rounded-md px-2 py-0.5 text-sm font-medium text-emerald-500 transition-all hover:bg-emerald-500/15 disabled:opacity-40"
              >
                {saving ? "saving…" : "save"}
              </button>
              <button
                onClick={cancelEdit}
                disabled={saving}
                className="rounded-md px-2 py-0.5 text-sm font-medium text-muted-foreground/60 transition-all hover:bg-white/5 hover:text-text-primary disabled:opacity-40"
              >
                cancel
              </button>
            </>
            ...
            <>
              <button
                onClick={startEdit}
                className="rounded-md p-1 text-muted-foreground/30 transition-all hover:bg-white/5 hover:text-text-primary"
                title="Edit guest"
              >
      ...
      <div className="divide-y divide-border/15">
        {/* phone input + WhatsApp / Telegram quick-message buttons.
        ...
        <div className="p-1.5">
          <div className="mb-1 flex items-center justify-between px-2 pt-1">
            ...
            <span className={`text-sm transition-opacity ${phoneState === "idle" ? "opacity-0" : "opacity-100"} ${phoneState === "saved" ? "text-emerald-500" : phoneState === "error" ? "text-destructive" : "text-muted-foreground/60"}`}>
              {phoneState === "saving"
                ? t.saving
                : phoneState === "saved"
                ? t.saved
          ...
          <div className="flex flex-wrap items-center gap-1.5 px-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-md border border-border/40 bg-background/50 px-2 py-1 text-sm text-text-primary placeholder-muted-foreground/30 focus:border-primary/60 focus:outline-none"
            />
            <a
          ...
          {phoneState === "idle" && phoneSavedValue === "" && (
            <p className="mt-1 px-2 text-sm text-muted-foreground/40">
              {tr("guest.phoneHelp")}
            </p>
          )}
        </div>
        {editing ? (
          <>
            <div className="p-1.5">
              <div className={cn("mb-0.5 px-2 pt-1", eyebrowVariants({ variant: "section" }))}>
                Identity
              </div>
            ...
            <div className="p-1.5">
              <div className="mb-0.5 px-2 pt-1 text-sm font-semibold uppercase tracking-widest text-emerald-500/70">
                Visa
              </div>
              <EditSelect
                label="Has visa"
          ...
          <>
            ...
            {childGuests.length > 0 && (
              <div className="p-1.5">
                <div className="mb-0.5 px-2 pt-1 text-sm font-semibold uppercase tracking-widest text-amber-400/70">
                ...
                {childGuests.map((child) => (
                  ...
                  >
                    <div className="flex items-center justify-between px-1">
                      ...
                      <div className="flex items-center gap-1">
                        <span className="text-sm text-muted-foreground/30">drag to move</span>
        ...
        {/* per-guest notes. Always visible (not gated by
        ...
        <div className="p-1.5">
          <div className="mb-1 flex items-center justify-between px-2 pt-1">
            ...
            <span className={`text-sm transition-opacity ${notesState === "idle" ? "opacity-0" : "opacity-100"} ${notesState === "saved" ? "text-emerald-500" : "text-muted-foreground/60"}`}>
          ...
          <textarea
            ...
            className="block w-full whitespace-pre-wrap rounded-md border border-border/40 bg-background/50 px-2 py-1.5 text-sm text-text-primary placeholder-muted-foreground/30 focus:border-primary/60 focus:outline-none"
            rows={2}
          />
        </div>
      </div>
  ...
  if (guests.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-border/25 py-12">
        ...
        <p className="text-caption">{tr("guest.noGuests")}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-caption">Click any value to copy</span>
      </div>
