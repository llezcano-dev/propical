    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <PageHeader
        ...
        actions={
          <button
            ...
            disabled={syncing}
            className="flex items-center gap-1.5 rounded-md bg-action-primary px-3.5 py-2 text-sm font-medium text-action-primary-fg transition-colors hover:bg-action-primary-hover disabled:opacity-50"
          >
      ...
      <div className="rounded-lg border border-border bg-surface-raised p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-text-primary">{t("tasks.autoSync")}</h2>
          {/* Toggle */}
          <button
            onClick={handleToggleAuto}
            className={`relative h-6 w-11 rounded-full transition-colors ${
              schedule.autoEnabled ? "bg-action-primary" : "bg-border-strong"
            }`}
          >
        ...
        {schedule.autoEnabled && (
          <>
            <div className="flex items-center gap-3">
              <span className="text-sm text-text-muted">{t("tasks.syncEvery")}</span>
            ...
            <div className="rounded-md border border-border-strong bg-surface p-3 space-y-2">
              <p className="text-sm font-medium text-text-muted">Self-hosters: trigger this URL from cron</p>
              {cronUrl ? (
                <code className="block text-data break-all select-all cursor-pointer rounded bg-surface-raised p-2 border border-border">{cronUrl}</code>
              ) : (
              ...
              <p className="text-caption">
                On the hosted instance this runs via Vercel Cron every 10 minutes. Self-hosters can wire it into <code className="text-text-muted">crontab</code> or any external scheduler (e.g. <a href="https://cron-job.org" target="_blank" rel="noopener noreferrer" className="text-text-primary hover:underline">cron-job.org</a>).
              </p>
            </div>
          </>
        )}
        ...
        <div className="border-t border-border pt-3 space-y-1.5">
          <div className="flex items-center gap-3">
            <span className="text-sm text-text-muted">{t("tasks.lastSync")}</span>
            {schedule.lastRun ? (
              <span className="text-sm text-text-primary">{timeSince(schedule.lastRun)}</span>
            ) : (
              <span className="text-sm text-text-faint">{t("tasks.never")}</span>
            )}
          </div>
          ...
          {parsedResult?.error && (
            <p className="text-sm text-rose-500">Error: {parsedResult.error}</p>
          )}

          {eventCounts.length > 0 && (
            <div className="border-t border-border/60 pt-3 mt-2 space-y-1">
              <p className="text-sm font-medium text-text-faint">Per-platform events</p>
              {eventCounts.map((p) => (
                <div key={p.id} className="flex items-center justify-between text-sm">
                  <span className="truncate text-text-secondary">{p.name}</span>
                  <span className="shrink-0 text-text-muted">
                    Airbnb:{" "}
                    <span className="font-semibold" style={{ color: resolvePlatformMeta("airbnb").color }}>
                      {p.airbnb}
                    </span>
                    {" · "}
                    Booking:{" "}
                    <span className="font-semibold" style={{ color: resolvePlatformMeta("booking").color }}>
                      {p.booking}
                    </span>
                  </span>
                </div>
      ...
      <div className="rounded-lg border border-border bg-surface-raised">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 className="text-sm font-medium text-text-muted">{t("tasks.syncLog")} ({logs.length})</h2>
          <div className="flex items-center gap-2">
            <select
              value={logLimit}
              onChange={(e) => setLogLimit(Number(e.target.value))}
              className="h-8 appearance-none rounded-md border border-border-strong bg-surface pl-2 pr-6 text-sm text-text-primary outline-none"
            >
            ...
            <button
              onClick={fetchData}
              className="rounded-md px-2 py-1 text-sm text-text-muted hover:bg-surface-hover hover:text-text-primary"
            >
              {t("common.refresh")}
            </button>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border text-left">
                  <th className={cn("px-4 py-2 w-[140px]", eyebrowVariants({ variant: "section" }))}>{t("tasks.time")}</th>
                  <th className={cn("px-4 py-2 w-[70px]", eyebrowVariants({ variant: "section" }))}>{t("tasks.level")}</th>
                  <th className={cn("px-4 py-2", eyebrowVariants({ variant: "section" }))}>{t("tasks.message")}</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b border-border/50 hover:bg-surface-hover">
                    <td className="px-4 py-2 text-sm text-text-faint whitespace-nowrap">
