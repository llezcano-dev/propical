  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(translateApiError(data, t) || t("signup.failed"));
        return;
      }
      setCode("");
      setStep("verify");
    ...
    <div className="editorial min-h-screen flex flex-col">
      ...
      <main className="flex flex-1 items-center justify-center px-6 py-10 sm:py-14">
        <div className="w-full max-w-[360px]">
          ...
          {signupEnabled === false ? (
            <div className="rounded-xl border border-border bg-surface-raised p-6 text-[14px] text-text-secondary">
              <p className="font-medium text-text-primary">Signups are temporarily disabled</p>
              <p className="mt-2 text-text-muted">
                We&apos;re not accepting new accounts right now. Please check back later
                {supportEmail ? (
                  <>
                    {" "}or contact{" "}
                    <a className="text-text-primary underline" href={`mailto:${supportEmail}`}>
                      {supportEmail}
                    </a>
                    .
                  </>
            ...
            <div className="rounded-xl border border-border bg-surface-raised p-6 sm:p-7">
              ...
              <div className="mt-4 flex items-center justify-between text-sm">
                <button
                  ...
                  disabled={loading}
                  className="text-text-muted underline-offset-2 hover:text-text-primary hover:underline disabled:opacity-50"
                >
                  {t("signup.resendCode")}
                </button>
                <button
