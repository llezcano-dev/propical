...
const COPY: Record<Locale, CopyShape> = {
  ...
  pt: {
    failedToAdd: "Não foi possível adicionar o usuário",
    subtitle: "Administradores e gestores desta instância. Profissionais de limpeza são adicionados por propriedade nas respectivas fichas.",
    loading: "Carregando...",
...
export default function AdminUsersPage() {
  ...
  const impersonate = async (id: number, username: string) => {
    if (!window.confirm(`View Propical as ${username}? You'll see exactly what they see; click "Exit impersonation" in the top banner to return to your admin session.`)) {
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <PageHeader title={tr("settings.users")} subtitle={t.subtitle} />
          ...
          <form onSubmit={addUser} className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <input
                ...
                className="h-10 rounded-md border border-border-strong bg-surface px-3 text-sm text-text-primary placeholder-text-faint outline-none transition-colors focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                required
              />
              <input
                type="password"
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-surface-raised">
        {!loaded ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={cn("border-b border-border text-left", eyebrowVariants({ variant: "section" }))}>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ${
                        u.role === "superadmin"
                          ? "bg-action-primary/15 text-action-primary-text"
                      ...
                      {u.role !== "superadmin" && (
                        <div className="flex items-center justify-end gap-0.5">
                          <button
                            ...
                            className="rounded-md p-1.5 text-text-faint transition-all hover:bg-amber-500/15 hover:text-amber-500"
                          ...
                          <button
                            ...
                            className="rounded-md p-1.5 text-text-faint transition-all hover:bg-rose-500/15 hover:text-rose-400"
