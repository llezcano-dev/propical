...
const COPY: Record<Locale, CopyShape> = {
  ...
  pt: {
    title: "Plataformas (calendário)",
    description:
      "Registro de plataformas de reservas. As cores e a ordem são aplicadas no calendário e no painel. O slug fica embutido na URL do feed iCal de saída (/for-{slug}.ics) e não pode ser alterado após a criação.",
    loading: "Carregando...",
    notSuperadmin: "Apenas superadministradores podem editar o registro de plataformas.",
    existingHeader: "Existentes",
    refreshing: "Atualizando...",
    refresh: "Atualizar",
    loadFailed: (status) => `Não foi possível carregar as plataformas (${status})`,
    loadFailedShort: "Não foi possível carregar as plataformas",
    empty: "Sem plataformas.",
    ...
    confirmDelete: (name, slug) => `Excluir a plataforma "${name}" (${slug})? Esta ação não pode ser desfeita.`,
    colHeaderName: "Nome",
    colHeaderColor: "Cor",
    colHeaderSort: "Ordem",
    colHeaderStatus: "Status",
    colHeaderActions: "Ações",
    enabled: "Ativa",
    disabled: "Inativa",
    save: "Salvar",
    delete: "Excluir",
    addCustomTitle: "Adicionar uma plataforma personalizada",
    addCustomDescription:
      "Use para plataformas que não estão na lista integrada. O slug é permanente: fica embutido na URL do feed iCal de saída /for-{slug}.ics.",
...
// Calendar platforms sub-route at
        ...
        <>
          <div className="overflow-hidden rounded-xl border border-border bg-surface-raised">
            ...
            {!error && rows.length === 0 && !loading && (
              <p className="px-4 pb-3 text-xs text-text-faint">
                {t.empty}
              </p>
            )}
            {rows.length > 0 && (
                      ...
                      return (
                        <tr key={p.slug} className={!draft.enabled ? "opacity-60" : ""}>
                          <td className="px-4 py-3 font-mono text-xs text-text-muted">
                            {p.slug}
                            {p.isCustom && (
                              <span className="ml-2 inline-flex items-center rounded-md bg-action-primary/15 px-1.5 py-0.5 text-sm font-medium uppercase tracking-wide text-action-primary-text">
                                {t.customBadge}
                              </span>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            <input
                              ...
                              maxLength={64}
                              className="h-8 w-40 rounded-md border border-border-strong bg-surface px-2 text-sm text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                            />
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                className="h-8 w-8 cursor-pointer rounded-md border border-border-strong bg-transparent"
                              ...
                              <input
                                ...
                                maxLength={7}
                                className="h-8 w-24 rounded-md border border-border-strong bg-surface px-2 font-mono text-xs uppercase text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                              />
                            </div>
                          </td>
                          <td className="px-4 py-3 text-right">
                            <input
                              ...
                              className="ml-auto h-8 w-20 rounded-md border border-border-strong bg-surface px-2 text-right text-sm tabular-nums text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                            />
                          </td>
                          <td className="px-4 py-3">
                            <select
                              ...
                              className="h-8 rounded-md border border-border-strong bg-surface px-2 text-xs text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                            >
                              <option value="true">
                                {t.enabled}
                              </option>
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="h-7 rounded-md bg-action-primary px-2.5 text-xs font-medium text-action-primary-fg transition-colors hover:bg-action-primary-hover disabled:opacity-50"
                              >
                                {busy === p.slug ? "…" : t.save}
                              </button>
                              {p.isCustom && (
          ...
          <div className="rounded-xl border border-border bg-surface-raised p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto_auto]">
              <div>
                <label className="mb-1 block text-xs text-text-faint" htmlFor="np-slug">
                  Slug
                </label>
                <input
                  id="np-slug"
                  ...
                  className="h-9 w-full rounded-md border border-border-strong bg-surface px-3 font-mono text-sm text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs text-text-faint" htmlFor="np-name">
                  {t.displayName}
                </label>
                <input
                  id="np-name"
                  ...
                  placeholder="My Platform"
                  className="h-9 w-full rounded-md border border-border-strong bg-surface px-3 text-sm text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs text-text-faint" htmlFor="np-color">
                  {t.color}
                </label>
                <input
                  id="np-color"
                  ...
                  className="h-9 w-16 cursor-pointer rounded-md border border-border-strong bg-transparent"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs text-text-faint" htmlFor="np-sort">
                  {t.sort}
                </label>
                <input
                  id="np-sort"
                  ...
                  className="h-9 w-20 rounded-md border border-border-strong bg-surface px-2 text-sm tabular-nums text-text-primary outline-none focus:border-text-primary focus:ring-1 focus:ring-text-primary/30"
                />
              </div>
