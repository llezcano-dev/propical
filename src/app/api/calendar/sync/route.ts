...
export async function POST(request: NextRequest) {
  try {
    ...
    if (propertyId != null && !Number.isNaN(propertyId)) {
      ...
      propertyIds = await listAccessiblePropertyIds(session.userId);
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const ownedIds = await listAccessiblePropertyIds(session.userId);

    if (propertyId) {
      const numId = Number(propertyId);
      if (!(await canReadProperty(numId, session.userId))) {
    ...
    // when a propertyId is supplied, drop global (null
