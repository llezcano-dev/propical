 ...
 * Called by Vercel Cron (vercel.json) every 10 min.
...
export async function GET(request: NextRequest) {
  ...
  await prisma.syncLog.create({
    data: {
      level: "info",
      message: syncLogMessage("sync.log.cronTriggered", { source: source.substring(0, 80) }),
    },
  });
  ...
  if (autoSetting?.value === "false") {
    await prisma.syncLog.create({
      data: { level: "info", message: syncLogMessage("sync.log.cronSkippedDisabled") },
    });
  ...
  if (lastRunSetting?.value) {
    ...
    if (elapsed < freqMinutes) {
      await prisma.syncLog.create({
        data: {
          level: "info",
          message: syncLogMessage("sync.log.cronSkippedRecent", {
  ...
  try {
    ...
    await prisma.syncLog.create({
      data: {
        level: "error",
        message: syncLogMessage("sync.log.cronError", { msg }),
      },
    });
