...
const ALLOWED_STRING_FIELDS = [
  ...
  "visaFrom",
  "visaTo",
  // per-guest free-text notes; preserves whitespace and
  // newlines so a paragraph from the host's clipboard round-trips.
  "notes",
] as const;

async function loadManageableGuest(guestId: number, userId: number) {
  ...
  if (!(await canManageProperty(guest.reservation.propertyId, userId))) return null;
  return guest;
}
...
export async function PATCH(
  ...
  try {
    ...
    const owned = await loadManageableGuest(numId, session.userId);
    ...
    // phone is sanitised separately so we can return a 400 on
