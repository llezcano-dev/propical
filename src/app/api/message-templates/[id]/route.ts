...
async function loadManageableTemplate(id: number, userId: number) {
  const t = await prisma.messageTemplate.findUnique({
    where: { id },
    select: { id: true, propertyId: true },
  });
  if (!t) return null;
  if (!(await canManageProperty(t.propertyId, userId))) return null;
  return t;
}
...
export async function PATCH(
  ...
  try {
    ...
    const owned = await loadManageableTemplate(numId, session.userId);
