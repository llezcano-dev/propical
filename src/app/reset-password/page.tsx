  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(translateApiError(data, t) || t("reset.failed"));
        return;
      }
      setStep("done");
    } catch {
          ...
          <div className="rounded-xl border border-border bg-surface-raised p-6 sm:p-7">
            ...
            {step === "done" && (
              <div className="text-center">
                ...
                <Link
                  href="/login"
                  className="inline-flex h-11 w-full items-center justify-center rounded-md bg-action-primary text-[14px] font-medium text-action-primary-fg transition-colors hover:bg-action-primary-hover"
                >
                  {t("login.signIn")}
                </Link>
              </div>
